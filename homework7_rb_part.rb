...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    ...
    elsif( (real_close(x1, x2) and (y2 < y1)) or (not real_close(x1, x2) and (x1 > x2)))
  ...
  def intersectWithSegmentAsLineResult seg
    if( real_close(@x1, seg.x1 ) )  # on the same VerticalLine
      if(@y1 < seg.y1)
        ax1, ay1, ax2, ay2, bx1, by1, bx2, by2 = @x1, @y1, @x2, @y2, seg.x1, seg.y1, seg.x2, seg.y2
      else
        ax1, ay1, ax2, ay2, bx1, by1, bx2, by2 = seg.x1, seg.y1, seg.x2, seg.y2, @x1, @y1, @x2, @y2
      end

      if( real_close(ay2, by1) )
        return Point.new(ax2, ay2)
      elsif (ay2 < by1 )
        return NoPoints.new
      elsif (ay2 > by2 )
        return LineSegment.new(bx1, by1, bx2, by2)
      else
        return LineSegment.new(bx1, by1, ax2, ay2)
      end
   ...
   end #end_intersetSegmentAsLineResult
end
